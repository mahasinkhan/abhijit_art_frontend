...
export default function ChatWidget() {
  ...
  return (
    <>
      {/* ── Panel ── */}
      {open && (
        <div className="cw-panel" role="dialog" aria-label="Chat with Avijit Art">
          {/* header */}
          <div className="cw-head">
            <div className="cw-avatar">A</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 800, fontSize: 15 }}>Avijit Art</div>
          ...
          <div className="cw-body">
            <div className="cw-bubble">👋 Hi there! Welcome to <b>Avijit Art</b>.</div>
      ...
      <style>{`
        .cw-fab {
          ...
          box-shadow: 0 12px 30px ${ACCENT}55; transition: transform .2s ease;
        }
        .cw-fab:hover { transform: translateY(-3px) scale(1.04); }
        .cw-ping { position: absolute; inset: 0; border-radius: 50%; box-shadow: 0 0 0 0 ${ACCENT}66; animation: cw-ping 2s infinite; }
        @keyframes cw-ping { 0% { box-shadow: 0 0 0 0 ${ACCENT}55; } 70% { box-shadow: 0 0 0 16px ${ACCENT}00; } 100% { box-shadow: 0 0 0 0 ${ACCENT}00; } }

        .cw-panel {
          position: fixed; bottom: 96px; right: 24px; z-index: 9999;
          width: 350px; max-width: calc(100vw - 32px); max-height: 72vh;
          background: ${BG}; border: 1px solid ${LINE}; border-radius: 18px; overflow: hidden;
          display: flex; flex-direction: column;
          box-shadow: 0 24px 60px rgba(42,35,29,0.22);
          font-family: 'DM Sans', system-ui, sans-serif;
          animation: cw-up .25s cubic-bezier(.2,.8,.2,1);
        }
        ...
        .cw-avatar { width: 40px; height: 40px; border-radius: 50%; background: rgba(255,255,255,.22);
        ...
        .cw-x { margin-left: auto; background: none; border: none; color: #fff; font-size: 26px; line-height: 1; cursor: pointer; opacity: .9; }

        .cw-body { padding: 18px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px; }
        .cw-bubble { background: #fff; border: 1px solid ${LINE}; color: ${INK}; padding: 11px 14px;
          border-radius: 14px 14px 14px 4px; font-size: 14px; line-height: 1.5; max-width: 92%; box-shadow: 0 2px 8px rgba(42,35,29,.04); }

        .cw-form { display: flex; flex-direction: column; gap: 10px; margin-top: 6px; }
        .cw-input { width: 100%; box-sizing: border-box; padding: 11px 14px; border-radius: 11px;
        ...
        .cw-send { width: 100%; padding: 12px; border: none; border-radius: 11px; cursor: pointer;
          background: ${ACCENT}; color: #fff; font-weight: 800; font-size: 14.5px; letter-spacing: .3px;
          box-shadow: 0 8px 20px ${ACCENT}40; transition: opacity .2s; }
        .cw-send:hover { opacity: .92; }
