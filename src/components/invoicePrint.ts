...
// All invoice print / PDF HTML builders, split out of InvoiceMaker.tsx.
// Pure functions: invoice data in → a full printable HTML document out.
// No React, no app imports. Each builder is self-contained (inline escape /
// number / amount-in-words helpers), so nothing else has to move.
//
//   buildFullA4HTML       — ONE bill filling a whole A4 portrait sheet   ← Billing "Download"
//   buildLandscapeA4HTML  — one bill, full A4 landscape (wide item table)
//   buildSideBySideA4HTML — 2 portrait bills side by side on A4 landscape
//   buildSingleHalfA4HTML — one bill on the top half of A4, bottom blank
//   buildTwoUpA4HTML      — 2 copies stacked on one A4 portrait sheet
//   buildInvoicePopupHTML — one bill on A5 portrait
//   amtWordsPreview       — Indian-numbering amount-in-words (UI preview)
...
/* ═══════════════════════════════════════════════════════════
   buildFullA4HTML — ONE invoice filling a whole A4 PORTRAIT sheet.
   Used by the Billing tab's "Download PDF". Header + bill-to at
   the top, the items table grows to fill the page, totals + QR +
   signature anchored at the bottom. 210mm × 297mm, single copy.
   ═══════════════════════════════════════════════════════════ */
...
.qr-img{width:36mm;height:36mm;object-fit:contain;border:.5px solid #ddd}
.qr-lbl{font-size:6.5pt;font-weight:700;color:#c56a3a;text-align:center}
.qr-badges{display:flex;gap:1mm;flex-wrap:wrap;justify-content:center}
.badge{font-size:4.5pt;font-weight:700;color:#c56a3a;border:.5px solid #e89a3c;padding:.3mm 1mm;background:#fff6ee}
.qr-upi{font-size:5pt;color:#555;text-align:center}
    ...
    <div class="bot">
      <div class="bot-l">
        ...
        <div class="qr-row">
          ...
          <div class="sig"><img src="/images/Signature.png" alt="Signature" style="height:12mm;width:auto;display:block;margin:0 auto 1mm" onerror="this.style.display='none'"/><div class="sig-line"></div><div class="sig-lbl">Authorised Signatory</div></div>
        </div>
      </div>
      <div class="bot-r">
...
/* ═══════════════════════════════════════════════════════════
   buildLandscapeA4HTML — full-page A4 LANDSCAPE invoice
   Items table owns the whole left side; totals / QR / signature
   live in a fixed right sidebar.
   ═══════════════════════════════════════════════════════════ */
...
.qr-img{width:30mm;height:30mm;object-fit:contain;border:.5px solid #ddd}
.qr-badges{display:flex;gap:.8mm;flex-wrap:wrap;justify-content:center}
.badge{font-size:4pt;font-weight:700;color:#c56a3a;border:.5px solid #e89a3c;padding:.2mm .8mm;background:#fff6ee}
.qr-upi{font-size:4.8pt;color:#555;text-align:center}
.sig{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;text-align:center}
  ...
  <div class="page">
    ...
    <div class="main">
      ...
      <div class="right">
        ...
        <div class="payrow">
          ${p.qrSrc?`<div class="qr-wrap"><div class="qr-lbl">Payment QR Code</div><img src="${e(p.qrSrc)}" class="qr-img"/><div class="qr-badges"><span class="badge">GPay</span><span class="badge">Paytm</span><span class="badge">PhonePe</span><span class="badge">UPI</span></div><div class="qr-upi">UPI: 9932913826@okbizaxis</div></div>`:""}
          <div class="sig"><img src="/images/Signature.png" alt="Signature" style="height:10mm;width:auto;display:block;margin:0 auto 1mm" onerror="this.style.display='none'"/><div class="sig-line"></div><div class="sig-lbl">Authorised Signatory</div></div>
        </div>
        <div class="thankyou">Thank you for your business!</div>
      </div>
    </div>
...
/* ═══════════════════════════════════════════════════════════
   buildSideBySideA4HTML — A4 LANDSCAPE, 2 portrait bills side by
   side with a cut line down the middle. Each bill 148.5mm wide.
   ═══════════════════════════════════════════════════════════ */
    ...
    <div class="bill">
      ...
      <div class="bot">
        <div class="bot-l">
          ...
          <div class="pay-row">
            ${p.qrSrc?`<div class="qr-wrap"><div class="qr-lbl">Payment QR</div><img src="${e(p.qrSrc)}" class="qr-img"/><div class="qr-upi">UPI: 9932913826@okbizaxis</div></div>`:""}
            <div class="sig"><img src="/images/Signature.png" style="height:9mm;width:auto;display:block;margin:0 auto 1mm" onerror="this.style.display='none'"/><div class="sig-line"></div><div class="sig-lbl">Authorised Signatory</div></div>
          </div>
        </div>
        <div class="bot-r">
...
.qr-img{width:22mm;height:22mm;object-fit:contain;border:.5px solid #ddd}
...
/* ═══════════════════════════════════════════════════
   buildSingleHalfA4HTML — 1 invoice on top half of A4,
   bottom half blank. Print → cut → 2 copies from 2 sheets.
   ═══════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════
   buildTwoUpA4HTML — 2 copies of the same bill stacked
   on one A4 portrait sheet, cut line between them.
   ═══════════════════════════════════════════════════ */
...
/* Amount-in-words for the live UI preview (Indian numbering). */
