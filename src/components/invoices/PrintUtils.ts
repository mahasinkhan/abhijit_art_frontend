...
// ── All invoice print / PDF logic isolated here ───────────────────────────
...
.qr-wrap{display:flex;flex-direction:column;align-items:center;gap:2mm}.qr-img{width:40mm;height:40mm;object-fit:contain;border:.5px solid #ddd}.qr-lbl{font-size:7pt;font-weight:700;color:#c56a3a;text-align:center}
...
export async function printInvoice(inv: Invoice) {
  ...
  const [qrB64, logoB64] = await Promise.all([toB64('/images/QR.jpeg'), toB64('/images/abhijit_art_logo.png')]);
  const biz  = (inv.business || {}) as any;
  const paid = effectivePaid(inv);
  const w = window.open("","_blank","width=820,height=1160");
  if (!w) return;
  const html = buildSingleHalfA4HTML({
