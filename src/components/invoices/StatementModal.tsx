...
import {
  CustomerRow, Invoice, InvStatus, STATUS_META, MONTH_NAMES,
  INK, MUTE, FAINT, TERRA, GREEN, LINE, SANS,
  num, round2, rupee as makeRupee, fmt, fmtTime, effectivePaid, sharedSt,
} from "./types";
...
export default function StatementModal({ row, onClose }: Props) {
  ...
  }, [row, stFrom, stTo, stStatus, stMonth, stYear]);
  ...
  const download = async () => {
    ...
    const rows = txns.map(x=>`<tr class="${x.kind}"><td>${esc(fmt(x.date))}<div class="tm">${esc(fmtTime(x.date))}</div></td><td><b>${esc(x.desc)}</b>${x.sub?`<div class="sub">${esc(x.sub)}</div>`:""}</td><td class="r">${x.debit?fmtN(x.debit):"—"}</td><td class="r cr">${x.credit?fmtN(x.credit):"—"}</td><td class="r bal">${fmtN(x.balance)}</td></tr>`).join("");
              ...
              {txns.length === 0
                ...
                : txns.map((x,i) => (
                  <tr key={i} style={x.kind==="payment"?{background:"#f6fbf7"}:undefined}>
                    <td style={{ ...st.td, whiteSpace:"nowrap" }}>{fmt(x.date)}<div style={{ fontSize:10.5, color:FAINT }}>{fmtTime(x.date)}</div></td>
...
const st: Record<string, React.CSSProperties> = {
  modal:    { width:"min(760px,100%)", maxHeight:"calc(100vh - 40px)", background:"#fffdfb", boxShadow:"0 30px 80px rgba(24,22,28,.34)", display:"flex", flexDirection:"column", overflow:"hidden", padding:"18px 20px" },
